...
public class JwsTransform(JwsOptions jwsOptions) : JoseTransformBase
{
    internal override async Task Decode(JoseTransformContext joseTransformContext)
    {
        ...
        {
            if (jwsOptions.ThrowOnNotSigned)
            {
        ...
        var result = await jwsOptions.Validator.Validate(joseTransformContext, jwsContent);

        transformParams.ValidationResult = result;

        if (jwsOptions.ThrowOnInvalid && !result.IsValid) throw new Exception("Invalid signature");
    ...
    internal override async Task Encode(JoseTransformContext joseTransformContext)
    {
        ...
        var jwsContent = await jwsOptions.Signer.Sign(joseTransformContext, step.Content, step.ContentType);

        SignatureFormat format = jwsOptions.SignatureFormat;
    ...
    private async Task<IJwsContent?> ParseJwsContent(JoseTransformContext context, string content, string[]? sigs)
    {
        if (sigs is { Length: >0 })
        {
            // detached
            ...
            var dSigs = sigs.Select(sig => {
                ...
                if (!(parts is { Length: 3 })){
                    if (jwsOptions.ThrowOnParseError) throw new Exception("Compact JWS did not have three parts");
                    return null;
                }
